...
interface Settings { preset: string; autoVerify: boolean; maxTokens: number; maxCost: number; inputPrice: number; outputPrice: number; maxSeconds: number }
interface State {
...
export function AgentTaskPanel({ conversationId, running, onResume }: { conversationId: string; running: boolean; onResume: () => void }) {
  ...
  return <section aria-label="Agent task controls" className="rounded-xl border border-border-subtle bg-surface-base/50 p-3 text-xs space-y-3">
    ...
    {settings && <details><summary className="cursor-pointer font-medium">Permissions, limits, and project memory</summary><div className="mt-3 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-2">{([['maxTokens', 'Token limit'], ['maxSeconds', 'Time limit (seconds)'], ['maxCost', 'Spending limit USD (0 = off)'], ['inputPrice', 'Input price USD / million tokens'], ['outputPrice', 'Output price USD / million tokens']] as const).map(([key, label]) => <label key={key} className="flex flex-col gap-1">{label}<input type="number" min="0" step="any" className={field} disabled={blocked} value={settings[key]} onChange={e => { dirty.current = true; setSettings({ ...settings, [key]: Number(e.target.value) }); }} /></label>)}</div>
      <p className="text-txt-tertiary">Enter provider prices for the selected model. Limits are checked between calls; an in-flight response can exceed them. Update prices when changing models.</p><button type="button" className={btn} disabled={blocked} onClick={() => act({ type: 'settings', settings })}>Save task settings</button>
