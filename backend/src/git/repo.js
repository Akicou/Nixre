...
const POST_RECEIVE_HOOK = [
  ...
  '  fi',
  '}',
  // HTTPS pushes set REMOTE_USER (smartHttp); SSH pushes set NIXRE_PUSHER
  // (nixre-git-shell). Core's own pushes (merges, web edits) have neither.
  'PUSHER="${REMOTE_USER:-${NIXRE_PUSHER:-}}"',
  'case "$PUSHER" in *[!A-Za-z0-9._-]*) PUSHER="";; esac',
  'while read old new ref; do',
  '  case "$ref" in',
  '  refs/heads/*) BRANCH="${ref#refs/heads/}";',
  '    BODY="{\\"space\\":\\"$SPACE\\",\\"repo\\":\\"$REPO\\",\\"branch\\":\\"$BRANCH\\",\\"ref\\":\\"$ref\\",\\"before\\":\\"$old\\",\\"after\\":\\"$new\\",\\"pusher\\":\\"${PUSHER:-webhook}\\"}";',
  '    notify "$BODY" || echo "nixre: could not notify core about $BRANCH - webhooks, auto-deploy and Actions did not run" >&2;;',
  '  refs/tags/*)',
  '    BODY="{\\"space\\":\\"$SPACE\\",\\"repo\\":\\"$REPO\\",\\"ref\\":\\"$ref\\",\\"before\\":\\"$old\\",\\"after\\":\\"$new\\",\\"pusher\\":\\"${PUSHER:-webhook}\\"}";',
  '    notify "$BODY" || echo "nixre: could not notify core about $ref - Actions did not run" >&2;;',
  '  esac',
  'done',
  'exit 0',
].join('\n');
