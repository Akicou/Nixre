...
#
# This used to `sed -i` the committed docker-compose.yml, which left the
# checkout permanently dirty — the next `git fetch` / fast-forward could then
# conflict, and every `git status` showed a mysterious modification. The
# override file is the supported mechanism and is gitignored.
OVERRIDE="docker-compose.override.yml"
if [ ! -f "$OVERRIDE" ]; then
  log "Creating $OVERRIDE (host port mapping core -> 127.0.0.1:3001)…"
  cat > "$OVERRIDE" <<'YAML'
# Host-local port mapping. Generated by update-nixre.sh; gitignored.
# The system caddy reverse-proxies to 127.0.0.1:3001, while the committed
# compose file publishes core on 127.0.0.1:3002.
services:
  nixre-core:
    ports:
      - "127.0.0.1:3001:3002"
YAML
  ...
  log "$OVERRIDE already present — leaving it alone."
fi
