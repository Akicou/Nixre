#!/usr/bin/env bash
# no-more-register.sh — close signups on this Nixre instance.
#
# Sets NIXRE_REGISTRATION_CLOSED=true in /opt/nixre/.env and recreates the
# nixre-core container so POST /api/v1/register returns 403. Existing
# sessions, logins, git traffic, and the assistant are unaffected; only new
# account creation is blocked. Reopen with ./allow-registering.sh.
#
...
if grep -q "^NIXRE_REGISTRATION_CLOSED=" "$ENV_FILE" 2>/dev/null; then
  sed -i "s/^NIXRE_REGISTRATION_CLOSED=.*/NIXRE_REGISTRATION_CLOSED=true/" "$ENV_FILE"
else
  printf "\nNIXRE_REGISTRATION_CLOSED=true\n" >> "$ENV_FILE"
fi
log "NIXRE_REGISTRATION_CLOSED=true written to $ENV_FILE"
...
if [ "$code" = "403" ]; then
  ...
  log "WARNING: register endpoint answered $code, expected 403 — check docker logs nixre-core." >&2
  exit 1
fi
